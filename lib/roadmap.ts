...
export const ROADMAP: RoadmapRelease[] = [
  {
    ...
    groups: [
      ...
      {
        heading: "The bot, actually switched on",
        icon: "Bot",
        blurb:
          "Every part of it is built. None of it is running, and that is a configuration job rather than a coding one.",
        items: [
          {
            title: "Set the Discord bot up for real",
            description:
              "`bot_all_config()` returns an empty object and the worker has never once checked in, so nothing is broken - nothing has ever been configured. Slash commands, buttons and modals run on Vercel and need credentials plus an interactions URL; the green Online dot, chat XP, automod, the live feed and the counter channels need the gateway worker in `bot/` running somewhere always-on, because a Vercel function cannot hold a WebSocket open. The full ordered checklist, and what has already been verified about the current state, is in `docs/discord-bot.md` under *Where this deployment actually stands*. Worth doing early rather than late: it is the largest piece of already-built retention machinery on the site, and it is the only thing here that can reach a player who has closed the tab.",
            status: "next",
          },
        ],
      },
